  ...
  const handleSendMessage = async () => {
    if (input.trim() && attempts < maxAttempts[difficulty]) {
      ...
      try {
        ...
        const res = await fetch(`https://api3.talkwith.tech/game-three?input_text=${encodedPrompt}`, requestOptions);
        if (!res.ok) {
        ...
        const aiMessage = { text: 'Interviewer has left in disgust.', isUser: false };
        setMessages((prevMessages) => [...prevMessages, aiMessage]);
      } finally {
        setLoading(false);
      }
